...
{
    $queries = [
        "CREATE TABLE IF NOT EXISTS courses ( id INT AUTO_INCREMENT PRIMARY KEY, course_code VARCHAR(30) NOT NULL UNIQUE, title VARCHAR(150) NOT NULL, description TEXT NULL, teacher_id INT NULL, status ENUM('active','archived') NOT NULL DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_course_teacher FOREIGN KEY (teacher_id) REFERENCES users(id) ON DELETE SET NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
        "CREATE TABLE IF NOT EXISTS enrollments ( id INT AUTO_INCREMENT PRIMARY KEY, course_id INT NOT NULL, student_id INT NOT NULL, enrolled_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY unique_enrollment (course_id, student_id), CONSTRAINT fk_enrollment_course FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE, CONSTRAINT fk_enrollment_student FOREIGN KEY (student_id) REFERENCES users(id) ON DELETE CASCADE ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
        "CREATE TABLE IF NOT EXISTS notifications ( id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(150) NOT NULL, message TEXT NOT NULL, audience ENUM('all','students','teachers') NOT NULL DEFAULT 'all', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
        "CREATE TABLE IF NOT EXISTS activity_logs ( id INT AUTO_INCREMENT PRIMARY KEY, user_id INT NULL, action_text VARCHAR(255) NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_activity_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    ];
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'create_course') {
        ...
        if (!mysqli_stmt_execute($statement)) {
            redirectWithMessage('error', 'Could not create the course. The course code may already exist.');
        }
        ...
        $noticeMessage = "$code — $title is now available in the LMS.";
    ...
    if ($action === 'enroll_student') {
        ...
        if (!mysqli_stmt_execute($statement)) {
            redirectWithMessage('error', 'This student is already enrolled in that course.');
        }
    ...
    if ($action === 'update_role') {
        ...
        $role = (int) ($_POST['usertype'] ?? -1);

        if (!in_array($role, [0, 1, 2], true) || $userId === $adminId) {
            redirectWithMessage('error', 'That role update is not allowed.');
        }
    ...
    if ($action === 'post_notification') {
        ...
        if ($title === '' || $message === '' || !in_array($audience, ['all', 'students', 'teachers'], true)) {
            redirectWithMessage('error', 'Provide a title, message, and audience.');
        }
...
<body>
    <div class="app-shell">
        <aside class="sidebar" id="sidebar">
            <a class="brand" href="#overview"><span class="brand-mark">J</span><span>Jidanao <b>LMS</b></span></a>
            <nav>
        ...
        <main>
            ...
            <section class="stats">
                <article>
                    <span class="stat-icon blue"><i class="fa-solid fa-user-graduate"></i></span>
                    <div>
                ...
                <article>
                    <span class="stat-icon purple"><i class="fa-solid fa-chalkboard-user"></i></span>
                    <div>
                        <small>Guest</small>
                ...
                <article>
                    <span class="stat-icon orange"><i class="fa-solid fa-book"></i></span>
                    <div>
            ...
            <section class="panel grade-panel">
                ...
                <div class="grade-grid">
                    <?php for ($i = 1; $i <= 6; $i++): $count = $gradeCounts[$i]; $pct = $maxGrade ? round(($count / $maxGrade) * 100) : 0; ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="post">
                                            ...
                                            <button class="icon-button" title="Change course status">
    ...
    <div class="modal" id="courseModal" aria-hidden="true">
        <div class="modal-card">
            ...
            <form method="post" class="stack-form">
                ...
                <label>
                    Course code
                    <input name="course_code" maxlength="30" required placeholder="e.g. MATH-101" />
                </label>
                <label>
                    Course title
                    <input name="title" maxlength="150" required placeholder="e.g. Foundations of Mathematics" />
                </label>
                <label>
                    Teacher
                ...
                <label>
                    Description
                    <textarea name="description" placeholder="What will learners gain from this course?"></textarea>
                </label>
